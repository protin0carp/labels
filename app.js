...
(function printButtonHardFix(){
  ...
  function runPrint(test){
    ...
    try{
      const result = fn(product, test ? 1 : copies(), !!test, { showPrice: showPrice() });
...
(function categoryLightPatch(){
  ...
  const oldClear = clearProductForm;
  ...
  renderProducts = function(){
    ...
    if(!list) return oldRenderProducts();
    list.innerHTML='';
    products
  ...
  const oldRenderManage = renderManage;
  renderManage = function(){
    ensureCategoryTabs();
    const host=document.getElementById('manageList');
    if(!host) return oldRenderManage();
