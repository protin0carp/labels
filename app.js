...
const defaultProducts = [
  {id:'p1',name:'شوكر كرانشو',description:'دجاج باربكيو 200G',calories:532,carbs:38,protein:68,fat:12,price:15,shelfLifeDays:1},
  {id:'p2',name:'براونيز برتزل',description:'بروتين شوكولاتة سولتد كراميل 200G',calories:227.6,carbs:9,protein:22,fat:11.5,price:18,shelfLifeDays:3},
  {id:'p3',name:'تشيز كيك',description:'تشيز كيك فراولة لايت 180G',calories:310,carbs:26,protein:18,fat:14,price:20,shelfLifeDays:2}
];
...
function renderProducts(){const q=(document.getElementById('searchInput')?.value||'').trim();const list=document.getElementById('productList');if(!list)return;list.innerHTML='';products.filter(p=>!q||p.name.includes(q)||p.description.includes(q)).forEach(p=>{const el=document.createElement('div');el.className='product-item '+(selected?.id===p.id?'active':'');el.innerHTML=`<h3>${p.name}</h3><p>${p.description}</p><div class="chips"><span>${p.price ?? ''} SR</span><span>F ${p.fat}</span><span>P ${p.protein}</span><span>C ${p.carbs}</span><span>Cal ${p.calories}</span></div>`;el.onclick=()=>{selected=p;window.selected=p;renderProducts();renderSelected();renderEditor();};list.appendChild(el);});}
...
function ensurePriceInput(){
  if(document.getElementById('pPrice')) return;
  const shelf = document.getElementById('pShelf');
  if(!shelf) return;
  const input=document.createElement('input');
  input.id='pPrice'; input.className='field'; input.placeholder='السعر SR'; input.type='number'; input.step='0.1';
...
document.addEventListener('DOMContentLoaded',async()=>{
  ensurePriceInput();ensureFontSizeDisplay();
