...
let settings = JSON.parse(localStorage.getItem(LABEL_KEY) || 'null') || structuredClone(defaultSettings);
...
function renderSelected(){if(!selected)return;selectedName.textContent=selected.name;selectedDesc.textContent=selected.description;selectedCalories.textContent=selected.calories;selectedCarbs.textContent=selected.carbs;selectedProtein.textContent=selected.protein;selectedFat.textContent=selected.fat;selectedExp.textContent='Exp:- '+expiryDate(selected.shelfLifeDays);}
...
document.addEventListener('DOMContentLoaded',()=>{document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>nav(b.dataset.view));document.getElementById('searchInput')?.addEventListener('input',renderProducts);document.getElementById('saveProductBtn')?.addEventListener('click',()=>{const p={id:'p'+Date.now(),name:pName.value.trim(),description:pDesc.value.trim(),calories:Number(pCalories.value),carbs:Number(pCarbs.value),protein:Number(pProtein.value),fat:Number(pFat.value),shelfLifeDays:Number(pShelf.value||1)};if(!p.name||!p.description)return alert('اكتب اسم الحلى والمحتويات');products.unshift(p);selected=p;saveProducts();renderProducts();renderSelected();renderManage();});document.getElementById('saveSettingsBtn')?.addEventListener('click',()=>{saveSettings();alert('تم حفظ إعدادات الملصق');});document.getElementById('resetSettingsBtn')?.addEventListener('click',()=>{settings=structuredClone(defaultSettings);saveSettings();renderEditor();});document.getElementById('moveUp')?.addEventListener('click',()=>moveSelected(0,-.5));document.getElementById('moveDown')?.addEventListener('click',()=>moveSelected(0,.5));document.getElementById('moveLeft')?.addEventListener('click',()=>moveSelected(-.5,0));document.getElementById('moveRight')?.addEventListener('click',()=>moveSelected(.5,0));document.getElementById('fontSizeRange')?.addEventListener('input',e=>{if(!selectedField)return;settings[selectedField].size=Number(e.target.value);renderEditor();});renderProducts();renderSelected();renderManage();renderEditor();});
