  ...
  products.forEach(p=>{
    ...
    el.innerHTML=`
      <div class="manage-info">
        <h3>${p.name}</h3>
        <p>${p.description}</p>
        <div class="chips"><span>${priceText(p)}</span><span>${p.calories} Cal</span><span>${p.carbs} C</span><span>${p.protein} P</span><span>${p.fat} F</span></div>
      </div>
...
document.addEventListener('DOMContentLoaded',async()=>{
  ensurePriceInput();ensureCopiesInput();ensureFontSizeDisplay();
...
/* PREMIUM_SAFE_PATCH_START */
(function premiumSafePatch(){
  const PC_FONT_ID = 'pc-noto-kufi-font';
  const PC_STYLE_ID = 'pc-premium-safe-style';

  function ensureNotoFont(){
    if(!document.getElementById(PC_FONT_ID)){
      const link=document.createElement('link');
      link.id=PC_FONT_ID;
      link.rel='stylesheet';
      link.href='https://fonts.googleapis.com/css2?family=Noto+Kufi+Arabic:wght@500;700;800;900&family=Cairo:wght@500;700;800;900&display=swap';
      document.head.appendChild(link);
    }
    if(!document.getElementById(PC_STYLE_ID)){
      const style=document.createElement('style');
      style.id=PC_STYLE_ID;
      style.textContent=`
/* PC_SR_AND_CLARITY_PATCH */
.riyal-icon{display:none!important}
.price-number{font-family:'Noto Kufi Arabic',Cairo,Arial,Tahoma,sans-serif!important;font-weight:700!important}
.label-field.desc,.label-field.name{
  font-family:'Noto Kufi Arabic',Cairo,Arial,Tahoma,sans-serif!important;
  font-weight:700!important;
  line-height:1.28!important;
  text-rendering:geometricPrecision;
}

        body,button,input,select,textarea,.label-field,.product-item,.chip,.chips{
          font-family:'Noto Kufi Arabic',Cairo,Arial,Tahoma,sans-serif !important;
        }
        #pDesc{
        ...
        .pc-category-tabs button.active{
          background:#caff39;
          color:#111;
        }
        .pc-manage-search{
  ...
  function getProductCategory(p){
    return p?.category || p?.section || p?.group || 'protein';
  }
  function normalizeProductCategory(p){
    // لا نعدل بيانات السحابة القديمة هنا؛ فقط نعرضها كـ بروتين وكارب إذا ما لها قسم.
    return p;
  }
  function arabicChips(p){
    const price = (p?.price === undefined || p?.price === null || p?.price === '') ? '—' : `${p.price} SR`;
    return `<span>${price}</span><span>السعرات ${p.calories}</span><span>كارب ${p.carbs}</span><span>بروتين ${p.protein}</span><span>الدهون ${p.fat}</span>`;
  }

  function ensureDescriptionTextarea(){
    const old=document.getElementById('pDesc');
    if(!old || old.tagName.toLowerCase()==='textarea') return;
    const ta=document.createElement('textarea');
    ta.id=old.id;
    ta.className=old.className || 'field';
    ta.placeholder=old.placeholder || 'اكتب المحتويات كاملة';
    ta.value=old.value || '';
    ta.rows=4;
    old.replaceWith(ta);
  }
  ...
  function ensureCategorySelect(){
    ...
    select.innerHTML='<option value="protein">بروتين وكارب</option><option value="yomelz">يوملز</option>';
  ...
  function ensureCategoryTabs(){
    ...
    if(manageList && !document.getElementById('pcManageSearch')){
      ...
      input.className='pc-manage-search';
      input.placeholder='بحث سريع لتعديل الأصناف...';
  ...
  function passCategory(p){
    return activeCategory==='all' || getProductCategory(p)===activeCategory;
  }

  // لا نقص المحتويات في المعاينة؛ خلي النص كامل يظهر.
  if(typeof fitText === 'function'){
    fitText = function(text){ return String(text || ''); };
  }

  // إلغاء أي تصغير تلقائي سابق.
  if(typeof autoFontSize === 'function'){
    autoFontSize = function(key,text,base){ return Number(base || 12); };
  }

  const oldClearProductForm = typeof clearProductForm==='function' ? clearProductForm : null;
  clearProductForm = function(){
    if(oldClearProductForm) oldClearProductForm();
    ensureDescriptionTextarea();
    ensureCategorySelect();
    setFieldValue('pCategory', activeCategory==='yomelz' ? 'yomelz' : 'protein');
  };

  const oldFillProductForm = typeof fillProductForm==='function' ? fillProductForm : null;
  fillProductForm = function(p){
    p=normalizeProductCategory(p);
    if(oldFillProductForm) oldFillProductForm(p);
    ensureDescriptionTextarea();
    ensureCategorySelect();
    setFieldValue('pCategory', getProductCategory(p));
    const desc=document.getElementById('pDesc');
    if(desc) desc.value=p.description || '';
  };

  const oldReadProductForm = typeof readProductForm==='function' ? readProductForm : null;
  readProductForm = function(){
    const p = oldReadProductForm ? oldReadProductForm() : {};
    p.category = getFieldValue('pCategory') || (activeCategory==='yomelz' ? 'yomelz' : 'protein');
    return normalizeProductCategory(p);
  };

  renderProducts = function(){
    ensureNotoFont();ensureCategoryTabs();
    products = products.map(normalizeProductCategory);
    const q=(document.getElementById('searchInput')?.value||'').trim();
    const list=document.getElementById('productList');
    if(!list)return;
    list.innerHTML='';
    products
      .filter(p=>passCategory(p))
      .filter(p=>!q || (p.name||'').includes(q) || (p.description||'').includes(q))
      .forEach(p=>{
        const el=document.createElement('div');
        el.className='product-item '+(selected?.id===p.id?'active':'');
        el.innerHTML=`<h3>${p.name}</h3><p>${p.description}</p><div class="chips">${arabicChips(p)}</div>`;
      ...
      .forEach(p=>{
        ...
        el.innerHTML=`
          <div class="manage-info">
            <h3>${p.name}</h3>
            <p>${p.description}</p>
            <div class="chips">${arabicChips(p)}</div>
          </div>
          <div class="manage-actions">
            <button type="button" class="mini edit-product">تعديل</button>
            <button type="button" class="mini danger delete-product">حذف</button>
  ...
  const oldRenderEditor = typeof renderEditor==='function' ? renderEditor : null;
  renderEditor = function(){
    ensureNotoFont();
    if(oldRenderEditor) oldRenderEditor();
  };
...
/* PREMIUM_SAFE_PATCH_END */



(function forceSRMode(){
  function apply(){
    const cur=document.getElementById('pCurrency');
    if(cur){
      cur.value='sr';
      cur.style.display='none';
      const label=cur.closest('label');
