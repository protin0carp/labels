...
function priceText(product){
  ...
  if(v === '') return '—';
  return product?.currencyMode === 'symbol' ? `${v} ﷼` : `${v} SR`;
}
function priceHtml(product){
  const v = product?.price ?? '';
  if(v === '') return '—';
  if(product?.currencyMode === 'symbol') {
    return `<span class="price-number">${v}</span><img class="riyal-icon" src="${RIYAL_MARK_SRC}" alt="ريال">`;
  }
...
(function premiumSafePatch(){
  ...
  function normalizeProductCategory(p){
    if(!p) return p;
    if(!p.category) p.category = getProductCategory(p);
    return p;
  }
  function arabicChips(p){
    return `<span>${priceText(p)}</span><span>السعرات ${p.calories}</span><span>كارب ${p.carbs}</span><span>بروتين ${p.protein}</span><span>الدهون ${p.fat}</span>`;
  }
