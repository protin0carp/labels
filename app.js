...
const defaultSettings = {
  description: { x: 35, y: 18.3, font: 9, maxWidth: 47, align: 'center' },
  calories:    { x: 13.2, y: 30.3, font: 11, maxWidth: 13, align: 'center' },
  carbs:       { x: 28.4, y: 30.3, font: 11, maxWidth: 13, align: 'center' },
  protein:     { x: 43.0, y: 30.3, font: 11, maxWidth: 13, align: 'center' },
  fat:         { x: 57.8, y: 30.3, font: 11, maxWidth: 13, align: 'center' },
  expiry:      { x: 29.8, y: 41.3, font: 8.2, maxWidth: 25, align: 'left' },
  name:        { x: 23.2, y: 46.0, font: 8.5, maxWidth: 34, align: 'center' }
};
...
function renderLabel(containerId, editable) {
  ...
  el.innerHTML = layers.map(layer => `<div class="label-text ${layer.key} ${['calories','carbs','protein','fat'].includes(layer.key)?'number':''} ${activeLayerKey===layer.key?'active':''}" data-key="${layer.key}" style="left:${mmToPx(layer.x)}px;top:${mmToPx(layer.y)}px;font-size:${ptToPx(layer.font)}px;max-width:${mmToPx(layer.maxWidth || 40)}px;text-align:${layer.align || 'center'}">${escapeHtml(layer.text)}</div>`).join('');
...
function enableDrag(container) {
  container.querySelectorAll('.label-text').forEach(node => {
    node.addEventListener('pointerdown', ev => {
      ...
      function move(e) {
        e.preventDefault();
        settings[activeLayerKey].x = clamp(start.lx + pxToMm(e.clientX - start.x), 0, 70);
        settings[activeLayerKey].y = clamp(start.ly + pxToMm(e.clientY - start.y), 0, 50);
        renderLabel('labelEditor', true); renderLabel('labelPreview', false);
      }
...
function mmToPx(mm){ return mm*8; } function pxToMm(px){ return px/8; } function ptToPx(pt){ return pt*1.333; }
