...
function ensurePrintPriceToggle(){
  ...
  wrap.style.cssText = 'display:flex;align-items:center;gap:8px;margin-top:10px;font-weight:800;cursor:pointer;';
  wrap.innerHTML = `
    <input id="showPricePrint" type="checkbox" checked style="width:18px;height:18px;accent-color:#caff39;">
    <span>إظهار السعر في الطباعة</span>
  `;
...
/* PREMIUM_WHITE_UI_START */
(function premiumWhiteUI(){
  const STYLE_ID = 'pc-premium-white-ui-v2';
  const FONT_ID = 'pc-noto-kufi-ui-font-v2';
  ...
  function addStyle(){
    ...
    style.textContent = `
      :root{
        ...
        --pc-soft:#f9fafb;
        --pc-green:#9ad322;
        --pc-green-dark:#77aa12;
        --pc-danger:#dc2626;
        --pc-radius:14px;
        --pc-shadow:0 8px 24px rgba(17,24,39,.06);
      }
      ...
      *{box-sizing:border-box}
      ...
      /* إزالة مظهر AI / التدرجات */
      [class*="gradient"], .hero, .banner{
        background:var(--pc-card)!important;
        box-shadow:var(--pc-shadow)!important;
      }

      .app,.container,.shell,.page,.view,.main,.content{
        background:var(--pc-bg)!important;
      }

      .card,.panel,.box,.section,.product-item,.manage-item,
      .editor,.settings-card,.print-card,.selected-card,
      .preview-card,.toolbar,.modal-content{
        background:var(--pc-card)!important;
      ...
      p,small,.muted,.hint,.sub,.description{
        color:var(--pc-muted)!important;
      }

      /* إخفاء النصوص التقنية/الشرح الطويل في قسم الطباعة */
      .technical,.debug,.log,.console,.print-help,.print-note,.printer-help,
      [data-debug],[data-note="print"],[data-role="print-help"]{
      ...
      input:focus,textarea:focus,select:focus{
        ...
        box-shadow:0 0 0 3px rgba(154,211,34,.18)!important;
      }
      ...
      button:hover,.btn:hover,.mini:hover{
        background:var(--pc-soft)!important;
      }

      button.primary,.btn.primary,#printBtn,#saveProductBtn,
      .print-btn,.save-btn,[data-action="print"],[data-action="save"]{
        background:var(--pc-green)!important;
        border-color:var(--pc-green)!important;
        color:#111!important;
      }

      button.danger,.mini.danger,.delete-product{
      ...
      /* تنقل */
      .nav,.tab,.pc-category-tabs button{
        background:#fff!important;
        border:1px solid var(--pc-border)!important;
        color:var(--pc-muted)!important;
      ...
      .nav.active,.tab.active,.pc-category-tabs button.active,
      .nav[aria-selected="true"],.tab[aria-selected="true"]{
        background:var(--pc-green)!important;
        border-color:var(--pc-green)!important;
        color:#111!important;
      ...
      /* قائمة الأصناف */
      #productList,.product-list,#manageList,.manage-list{
        display:grid;
        gap:10px;
      }

      .product-item,.manage-item{
        padding:14px!important;
        transition:.15s ease;
      }

      .product-item:hover,.manage-item:hover{
      ...
      .product-item.active{
        border-color:var(--pc-green)!important;
        box-shadow:0 0 0 3px rgba(154,211,34,.14)!important;
      }

      .product-item h3,.manage-info h3{
      ...
      /* منطقة الطباعة: بدون شروحات */
      #printPanel,#view-print,.print-section,.printer-section{
        background:var(--pc-bg)!important;
      }
      ...
      /* المعاينة */
      .label-preview,.preview,.label-box{
        background:#fff!important;
        border:1px solid var(--pc-border)!important;
        border-radius:12px!important;
      }

      /* نخفف أي ألوان قوية غير مرغوبة */
      .purple,.blue,.pink,.cyan,
      [class*="purple"],[class*="blue"],[class*="pink"],[class*="cyan"]{
        background:var(--pc-card)!important;
        color:var(--pc-text)!important;
      }
  ...
  function cleanPrintTexts(){
    const words = ['استدعاء الطابعة','تجهيز الطباعة','إرسال المهمة','أوامر الطابعة','جاري الاتصال','Rendering','Print job','Debug','Console'];
  ...
  function compactPrintArea(){
    const copies = document.getElementById('copiesInput');
    if(copies){
      copies.placeholder = 'عدد النسخ';
    }
    const priceWrap = document.getElementById('showPricePrintWrap');
    if(priceWrap){
      const span = priceWrap.querySelector('span');
      if(span) span.textContent = 'السعر';
    }
  }
