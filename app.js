...
/* PRINT_BUTTON_HARD_FIX_START */
(function printButtonHardFix(){
  function readCopies(){
    const el = document.getElementById('copiesInput');
  ...
  function doPrint(test=false){
    const product = currentProduct();
    if(!product){
      alert('اختر الصنف أولاً');
      return;
    }
    if(typeof window.buildPrintDocument !== 'function' && typeof buildPrintDocument !== 'function'){
      alert('ملف الطباعة غير محمل. تأكد من رفع printer.js');
      return;
    }
    const fn = window.buildPrintDocument || buildPrintDocument;
  ...
  document.addEventListener('click', function(e){
    const printBtn = e.target.closest && e.target.closest('#printBtn');
    const testBtn = e.target.closest && e.target.closest('#testPrintBtn');
    if(!printBtn && !testBtn) return;
    e.preventDefault();
    e.stopPropagation();
    if(e.stopImmediatePropagation) e.stopImmediatePropagation();
    doPrint(Boolean(testBtn));
  }, true);
})();
/* PRINT_BUTTON_HARD_FIX_END */
