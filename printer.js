...
function pcBuildSingleLabel(product, settings, imageCache, showPrice = true) {
  ...
  return `
    <div class="label">
      ...
      ${showPrice ? `<div class="field price" style="${pcFieldCss(priceSettings, 'direction:ltr;')}">${pcPriceHtml(product)}</div>` : ``}
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('printBtn')?.addEventListener('click', () => {
    ...
    buildPrintDocument(product, copies, false, { showPrice: document.getElementById('showPricePrint')?.checked !== false });
  });
  ...
  document.getElementById('testPrintBtn')?.addEventListener('click', () => {
    ...
    buildPrintDocument(product, 1, true, { showPrice: document.getElementById('showPricePrint')?.checked !== false });
  });
});
