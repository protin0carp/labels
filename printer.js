...
function pcMakeDynamicTextImage(text, settings, options = {}) {
  ...
  // المهم: حجم الصورة مرتبط بحجم الخط والعرض من ضبط الملصق
  // فلا تصغر الصورة عند التكبير من ضبط الملصق.
  const qualityScale = 14;
  const pxPerPercent = Number(options.pxPerPercent || 10);
  const canvasW = Math.max(size * 8, Math.round(widthPercent * pxPerPercent));
  const lineHeight = size * Number(options.lineHeight || 1.32);
  const canvasH = Math.max(size * 2.4, Math.ceil((lineHeight * Math.max(1, lines.length)) + size * 1.1));

  const canvas = document.createElement('canvas');
  canvas.width = Math.ceil(canvasW * qualityScale);
  canvas.height = Math.ceil(canvasH * qualityScale);
  ...
  lines.forEach((line, i) => {
    ctx.fillText(String(line || ''), canvasW / 2, startY + i * lineHeight, canvasW - 8);
  });
