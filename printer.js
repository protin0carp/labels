...
async function pcEnsureCairoFont() {
  try {
    // يجبر المتصفح يحمّل خط Cairo قبل تحويل العربي إلى صورة Canvas.
    // بدون هذه الخطوة قد يرسم Canvas الخط الاحتياطي ويظهر نفس الخط القديم.
    if (document.fonts && document.fonts.load) {
      await document.fonts.load('900 16px "Cairo"');
      await document.fonts.load('800 16px "Cairo"');
...
function pcPriceHtml(product) {
  const text = pcPriceText(product);
  return text ? pcEscapeHtml(text) : '';
}
...
async function pcEnsurePrintFonts() {
  try {
    if (document.fonts && document.fonts.load) {
      await document.fonts.load('700 20px "Noto Kufi Arabic"');
      await document.fonts.load('800 20px "Noto Kufi Arabic"');
      await document.fonts.ready;
    }
  } catch (_) {}
}

function pcSplitForDynamicImage(text, maxChars = 32, maxLines = 2) {
...
function pcMakeDynamicTextImage(text, settings, options = {}) {
  const value = String(text ?? '').trim();
  const size = Math.max(5, Number(settings?.size || options.size || 12));
  const widthPercent = Math.max(5, Math.min(100, Number(settings?.width || options.widthPercent || 24)));
  const weight = Number(options.weight || 700);
  const direction = options.direction || 'rtl';
  const maxLines = Number(options.maxLines || 1);
  const maxChars = Number(options.maxChars || 30);
  const lines = options.lines || (maxLines > 1 ? pcSplitForDynamicImage(value, maxChars, maxLines) : [value]);

  // مهم جداً:
  // العرض المرئي للصورة في الطباعة لازم يساوي تقريباً نفس عرض العنصر داخل ملصق 55mm.
  // 55mm ≈ 208px في CSS. لو جعلنا canvasW كبير جداً ثم عرضناه بنسبة صغيرة، النص ينضغط ويطلع صغير.
  const labelCssPx = Number(options.labelCssPx || 208);
  const displayW = Math.max(size * 3.8, Math.round(labelCssPx * (widthPercent / 100)));
  const lineHeight = size * Number(options.lineHeight || 1.32);
  const displayH = Math.max(size * 2.15, Math.ceil((lineHeight * Math.max(1, lines.length)) + size * 0.95));

  // الجودة العالية تكون من scale فقط، وليس من تكبير العرض المرئي.
  const qualityScale = 14;
  const canvas = document.createElement('canvas');
  canvas.width = Math.ceil(displayW * qualityScale);
  canvas.height = Math.ceil(displayH * qualityScale);
  ...
  lines.forEach((line, i) => {
    ctx.fillText(String(line || ''), displayW / 2, startY + i * lineHeight, displayW - 4);
  });
...
function pcImageField(className, settings, img, extra = '') {
  if (!img || !img.src) return '';
  const w = Math.max(8, Number(img.displayW || 60));
  const h = Math.max(6, Number(img.displayH || 20));
  return `<img class="field text-img ${className}" style="${pcFieldCss(settings, `width:${w}px;height:${h}px;${extra}`)}" src="${img.src}" alt="">`;
}
...
function pcBuildSingleLabel(product, settings, imageCache) {
  ...
  return `
    <div class="label">
      ${pcImageField('desc-img', settings.desc, imageCache.desc)}
      ...
      ${pcImageField('price-img', priceSettings, imageCache.price, 'direction:ltr;')}
      ${pcImageField('name-img', settings.name, imageCache.name)}
    </div>`;
}

async function buildPrintDocument(product, copies = 1, test = false, options = {}) {
  const data = test
  ...
  const priceSettings = settings.price || settings.expiry;
  const imageCache = {
    desc: pcMakeDynamicTextImage(data.description || '', settings.desc, {
      weight: 700,
      direction: 'rtl',
      maxLines: 2,
      maxChars: 34,
    ...
    price: (showPrice && pcPriceText(data)) ? pcMakeDynamicTextImage(pcPriceText(data), priceSettings, {
      weight: 700,
      direction: 'ltr',
      widthPercent: 26,
      maxLines: 1,
    ...
    name: pcMakeDynamicTextImage(data.name || '', settings.name, {
      weight: 700,
      direction: 'rtl',
      maxLines: 2,
      maxChars: 14,
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('printBtn')?.addEventListener('click', () => {
    ...
    buildPrintDocument(product, copies, false, { showPrice: document.getElementById('showPricePrint')?.checked !== false }).catch(err=>{ console.error(err); alert('تعذر تجهيز الطباعة.'); });
  });
  ...
  document.getElementById('testPrintBtn')?.addEventListener('click', () => {
    ...
    buildPrintDocument(product, 1, true, { showPrice: document.getElementById('showPricePrint')?.checked !== false }).catch(err=>{ console.error(err); alert('تعذر تجهيز طباعة الاختبار.'); });
  });
});
