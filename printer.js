function pcEscapeHtml(value) {
  return String(value ?? '')
    .replaceAll('&', '&amp;')
    .replaceAll('<', '&lt;')
    .replaceAll('>', '&gt;')
    .replaceAll('"', '&quot;')
    .replaceAll("'", '&#039;');
}

// رمز الريال كصورة مضمّنة حتى لا يظهر مربع داخل الطباعة.
const PC_RIYAL_MARK_SRC = 'data:image/png;base64,';


async function pcEnsureCairoFont() {
  try {
    // يجبر المتصفح يحمّل خط Cairo قبل تحويل العربي إلى صورة Canvas.
    // بدون هذه الخطوة قد يرسم Canvas الخط الاحتياطي ويظهر نفس الخط القديم.
    if (document.fonts && document.fonts.load) {
      await document.fonts.load('900 16px "Cairo"');
      await document.fonts.load('800 16px "Cairo"');
      await document.fonts.ready;
    }
  } catch (_) {}
}

function pcGetSettings() {
  const defaults = {
    desc: { x: 50, y: 31, size: 16, width: 78 },
    calories: { x: 20, y: 50, size: 10 },
    carbs: { x: 45, y: 50, size: 10 },
    protein: { x: 67, y: 50, size: 10 },
    fat: { x: 87, y: 50, size: 10 },
    expiry: { x: 22, y: 82, size: 9 },
    name: { x: 14, y: 92, size: 13, width: 28 }
  };

  try {
    const saved = JSON.parse(localStorage.getItem('pc_label_settings_landscape_v1') || 'null');
    return saved ? { ...defaults, ...saved } : defaults;
  } catch (_) {
    return defaults;
  }
}

function pcFieldCss(s, extra = '') {
  const width = s.width ? `width:${s.width}%;` : '';
  return `left:${s.x}%;top:${s.y}%;font-size:${s.size}px;${width}${extra}`;
}

function pcSplitArabicLines(text) {
  text = String(text || '').trim();
  if (!text) return [''];
  if (text.length <= 24) return [text];

  const maxLines = text.length > 58 ? 3 : 2;
  const words = text.split(/\s+/).filter(Boolean);
  const lines = [];
  let current = '';
  const target = Math.ceil(text.length / maxLines);

  for (const word of words) {
    const next = (current + ' ' + word).trim();
    if (next.length > target && lines.length < maxLines - 1) {
      if (current) lines.push(current.trim());
      current = word;
    } else {
      current = next;
    }
  }
  if (current) lines.push(current.trim());
  return lines.slice(0, maxLines);
}

function pcAutoArabicSize(key, text, base) {
  // لا يوجد تصغير تلقائي. الحجم من ضبط الملصق فقط.
  return Number(base || 12);
}

function pcArabicImage(text, key, baseSize, widthPercent) {
  const lines = pcSplitArabicLines(text);
  const size = pcAutoArabicSize(key, text, baseSize);

  const scale = 5;
  const w = Math.max(130, Math.round((widthPercent || 40) * 5.4));
  const h = Math.max(38, Math.ceil((size * 1.42 * lines.length) + 12));

  const canvas = document.createElement('canvas');
  canvas.width = w * scale;
  canvas.height = h * scale;

  const ctx = canvas.getContext('2d');
  ctx.scale(scale, scale);
  ctx.clearRect(0, 0, w, h);
  ctx.fillStyle = '#000';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.direction = 'rtl';
  ctx.font = `900 ${size}px "Cairo"`;

  const lineHeight = size * 1.25;
  const startY = h / 2 - ((lines.length - 1) * lineHeight) / 2;

  lines.forEach((line, i) => {
    ctx.fillText(line, w / 2, startY + i * lineHeight, w - 8);
  });

  return {
    src: canvas.toDataURL('image/png'),
    widthPercent: widthPercent || 40,
    size
  };
}

function pcPriceHtml(product) {
  const text = pcPriceText(product);
  return text ? pcEscapeHtml(text) : '';
}


async function pcEnsurePrintFonts() {
  try {
    if (document.fonts && document.fonts.load) {
      await document.fonts.load('700 20px "Noto Kufi Arabic"');
      await document.fonts.load('800 20px "Noto Kufi Arabic"');
      await document.fonts.ready;
    }
  } catch (_) {}
}

function pcMakeTextImage(text, options = {}) {
  const value = String(text ?? '').trim();
  const size = Math.max(5, Number(options.size || 12));
  const weight = Number(options.weight || 700);
  const widthPercent = Math.max(5, Number(options.widthPercent || 24));
  const direction = options.direction || 'rtl';
  const lines = Array.isArray(options.lines) ? options.lines : String(value).split('\n');

  const scale = 14; // جودة عالية للطابعة 203dpi
  const w = Math.max(90, Math.round(widthPercent * 9));
  const lineHeight = size * Number(options.lineHeight || 1.35);
  const h = Math.max(34, Math.ceil((lineHeight * Math.max(1, lines.length)) + size * 1.2));

  const canvas = document.createElement('canvas');
  canvas.width = w * scale;
  canvas.height = h * scale;

  const ctx = canvas.getContext('2d', { alpha: true });
  ctx.scale(scale, scale);
  ctx.clearRect(0, 0, w, h);

  ctx.fillStyle = '#000';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.direction = direction;
  ctx.font = `${weight} ${size}px "Noto Kufi Arabic", Cairo, Arial, Tahoma, sans-serif`;

  const startY = h / 2 - ((lines.length - 1) * lineHeight) / 2;
  lines.forEach((line, i) => {
    ctx.fillText(String(line || ''), w / 2, startY + i * lineHeight, w - 8);
  });

  return {
    src: canvas.toDataURL('image/png'),
    widthPercent,
    heightPx: h
  };
}

function pcSplitForImage(text, maxChars = 30, maxLines = 2) {
  const value = String(text || '').trim();
  if (!value) return [''];
  const words = value.split(/\s+/).filter(Boolean);
  const lines = [];
  let current = '';
  for (const word of words) {
    const next = (current + ' ' + word).trim();
    if (next.length > maxChars && lines.length < maxLines - 1) {
      if (current) lines.push(current);
      current = word;
    } else {
      current = next;
    }
  }
  if (current) lines.push(current);
  return lines.slice(0, maxLines);
}

function pcPriceText(product) {
  const raw = product?.price;
  if (raw === undefined || raw === null || raw === '') return '';
  return `${raw} SR`;
}

function pcImageField(className, settings, img, extra = '') {
  if (!img || !img.src) return '';
  return `<img class="field text-img ${className}" style="${pcFieldCss(settings, `width:${img.widthPercent}%;${extra}`)}" src="${img.src}" alt="">`;
}

function pcBuildSingleLabel(product, settings, imageCache) {
  const priceSettings = settings.price || settings.expiry;

  return `
    <div class="label">
      ${pcImageField('desc-img', settings.desc, imageCache.desc)}
      ${pcImageField('calories-img', settings.calories, imageCache.calories, 'direction:ltr;')}
      ${pcImageField('carbs-img', settings.carbs, imageCache.carbs, 'direction:ltr;')}
      ${pcImageField('protein-img', settings.protein, imageCache.protein, 'direction:ltr;')}
      ${pcImageField('fat-img', settings.fat, imageCache.fat, 'direction:ltr;')}
      ${pcImageField('price-img', priceSettings, imageCache.price, 'direction:ltr;')}
      ${pcImageField('name-img', settings.name, imageCache.name)}
    </div>`;
}

async function buildPrintDocument(product, copies = 1, test = false) {
  const data = test
    ? { name: 'سولتد شوكلت', description: 'حمص - دارك شوكلت - بروتين شوكلت ستيفيا - حليب خالي من اللاكتوز - بندق', calories: 197.4, carbs: 15, protein: 12.3, fat: 9.8, price: 15, currencyMode: 'symbol' }
    : product;

  if (!data) {
    alert('اختر الصنف أولاً');
    return;
  }

  await pcEnsurePrintFonts();

  const count = Math.max(1, Number(copies || 1));
  const settings = pcGetSettings();

  const priceSettings = settings.price || settings.expiry;
  const imageCache = {
    desc: pcMakeTextImage(data.description || '', {
      size: settings.desc?.size || 15,
      weight: 700,
      widthPercent: settings.desc?.width || 78,
      direction: 'rtl',
      lines: pcSplitForImage(data.description || '', 34, 2),
      lineHeight: 1.32
    }),
    calories: pcMakeTextImage(data.calories ?? '', {
      size: settings.calories?.size || 10,
      weight: 700,
      widthPercent: 14,
      direction: 'ltr'
    }),
    carbs: pcMakeTextImage(data.carbs ?? '', {
      size: settings.carbs?.size || 10,
      weight: 700,
      widthPercent: 14,
      direction: 'ltr'
    }),
    protein: pcMakeTextImage(data.protein ?? '', {
      size: settings.protein?.size || 10,
      weight: 700,
      widthPercent: 14,
      direction: 'ltr'
    }),
    fat: pcMakeTextImage(data.fat ?? '', {
      size: settings.fat?.size || 10,
      weight: 700,
      widthPercent: 14,
      direction: 'ltr'
    }),
    price: pcPriceText(data) ? pcMakeTextImage(pcPriceText(data), {
      size: priceSettings?.size || 11,
      weight: 700,
      widthPercent: 26,
      direction: 'ltr'
    }) : null,
    name: pcMakeTextImage(data.name || '', {
      size: settings.name?.size || 13,
      weight: 700,
      widthPercent: settings.name?.width || 28,
      direction: 'rtl',
      lines: pcSplitForImage(data.name || '', 14, 2),
      lineHeight: 1.2
    })
  };

  let labels = '';
  for (let i = 0; i < count; i++) labels += pcBuildSingleLabel(data, settings, imageCache);

  const printWin = window.open('', '_blank', 'width=700,height=420');
  if (!printWin) {
    alert('المتصفح منع نافذة الطباعة. اسمح بالنوافذ المنبثقة.');
    return;
  }

  printWin.document.open();
  printWin.document.write(`<!doctype html>
<html lang="ar" dir="rtl">
<head>
<meta charset="utf-8">
<title>Print Label</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Noto+Kufi+Arabic:wght@500;700;800;900&family=Cairo:wght@500;700;800;900&display=swap');
  @page { size: 55mm 33.6mm; margin: 0; }
  html, body {
    width: 55mm;
    height: 33.6mm;
    margin: 0;
    padding: 0;
    background: #fff;
    overflow: hidden;
    font-family: 'Noto Kufi Arabic', Cairo, Arial, Tahoma, sans-serif;
  }
  * { box-sizing: border-box; }
  .label {
    position: relative;
    width: 55mm;
    height: 33.6mm;
    margin: 0;
    padding: 0;
    overflow: hidden;
    background: transparent;
    page-break-after: always;
    break-after: page;
  }
  .text-img {
    display: block;
    height: auto;
    image-rendering: auto;
  }
  .field {
    position: absolute;
    transform: translate(-50%, -50%);
    color: #000;
    font-family: 'Noto Kufi Arabic', Cairo, Arial, Tahoma, sans-serif;
    font-weight: 700;
    text-align: center;
    line-height: 1.18;
    white-space: normal;
    -webkit-print-color-adjust: exact;
    print-color-adjust: exact;
    text-rendering: geometricPrecision;
    -webkit-font-smoothing: antialiased;
  }
  .arabic-img {
    display: block;
    height: auto;
    image-rendering: auto;
  }
  .desc, .name {
    font-family: 'Noto Kufi Arabic', Cairo, Arial, Tahoma, sans-serif;
    font-weight: 700;
    direction: rtl;
    line-height: 1.28;
    white-space: normal;
    letter-spacing: 0;
    text-rendering: geometricPrecision;
    -webkit-font-smoothing: antialiased;
  }
  .num {
    font-size: 11px;
    font-weight: 700;
    direction: ltr;
    font-family: 'Noto Kufi Arabic', Cairo, Arial, Tahoma, sans-serif;
  }
  .price {
    font-size: 12.5px;
    font-weight: 700;
    direction: ltr;
    white-space: nowrap;
    font-family: 'Noto Kufi Arabic', Cairo, Arial, Tahoma, sans-serif;
  }
  .price-number {
    display: inline-block;
    vertical-align: middle;
    line-height: 1;
  }
  .riyal-icon { display:none!important; }
  @media print {
    html, body { width:55mm; height:33.6mm; margin:0; padding:0; overflow:hidden; }
    .label { width:55mm; height:33.6mm; }
  }
</style>
</head>
<body>
${labels}
<script>
  window.onload = function () {
    setTimeout(function () {
      window.focus();
      window.print();
    }, 350);
  };
  window.onafterprint = function () {
    setTimeout(function () { window.close(); }, 300);
  };
<\/script>
</body>
</html>`);
  printWin.document.close();
}

document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('printBtn')?.addEventListener('click', () => {
    const copies = Math.max(1, Number(document.getElementById('copiesInput')?.value || 1));
    const product = (typeof window.getSelectedProduct === 'function')
      ? window.getSelectedProduct()
      : (window.selected || (typeof selected !== 'undefined' ? selected : null));
    buildPrintDocument(product, copies, false);
  });

  document.getElementById('testPrintBtn')?.addEventListener('click', () => {
    const product = (typeof window.getSelectedProduct === 'function') ? window.getSelectedProduct() : (window.selected || null);
    buildPrintDocument(product, 1, true);
  });
});
