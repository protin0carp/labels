...
function pcPriceHtml(product) {
  ...
  if (product?.currencyMode === 'symbol') {
    return `<span class="price-number">${pcEscapeHtml(v)}</span><img class="riyal-icon" src="${PC_RIYAL_MARK_SRC}" alt="ريال">`;
  }
  return `<span class="price-number">${pcEscapeHtml(v)} SR</span>`;
}

function pcFitText(text, max = 44) {
  text = String(text || '').trim();
  const safe = pcEscapeHtml(text);
  if (text.length <= 22) return { html: safe, size: 11 };
  if (text.length <= 36) return { html: safe, size: 10 };
  ...
  return { html: first + '<br>' + second, size: 8.8 };
}
...
function pcAutoFontSize(key, text, base) {
...
function pcBuildSingleLabel(product, settings) {
  ...
  const descSize = Math.max(pcAutoFontSize('desc', product.description || '', settings.desc?.size), desc.size);
  const nameSize = pcAutoFontSize('name', product.name || '', settings.name?.size);
  return `
    <div class="label">
...
function buildPrintDocument(product, copies = 1, test = false) {
  const data = test
    ? { name: 'اختبار وضوح', description: 'قهوه لوز 200G', calories: 197.4, carbs: 15, protein: 12.3, fat: 9.8, price: 15, currencyMode: 'symbol', shelfLifeDays: 1 }
    : product;
  if (!data) { alert('اختر الصنف أولاً'); return; }
  ...
  const oldFrame = document.getElementById('pcPrintFrame');
  if (oldFrame) oldFrame.remove();
  const iframe = document.createElement('iframe');
  iframe.id = 'pcPrintFrame';
  iframe.style.position = 'fixed';
  iframe.style.left = '-10000px';
  iframe.style.top = '0';
  iframe.style.width = '55mm';
  iframe.style.height = '33.6mm';
...
<style>
  ...
  html, body {
    width: 55mm; height: 33.6mm; margin: 0; padding: 0; background: #fff; overflow: hidden;
    font-family: Cairo, Tahoma, Arial, sans-serif;
  }
  * { box-sizing: border-box; }
  .label {
    position: relative; width: 55mm; height: 33.6mm; margin: 0; padding: 0; overflow: hidden;
    background: transparent; page-break-after: always; break-after: page;
  }
  .field {
    position: absolute; transform: translate(-50%, -50%); color: #000;
    font-family: Cairo, Tahoma, Arial, sans-serif; font-weight: 900;
    text-align: center; line-height: 1.16; white-space: normal;
    -webkit-print-color-adjust: exact; print-color-adjust: exact;
    -webkit-font-smoothing: antialiased; text-rendering: optimizeLegibility;
  }
  .desc, .name { font-family: Cairo, Tahoma, Arial, sans-serif; font-weight: 900; line-height: 1.2; }
  .num, .price { font-family: Cairo, Tahoma, Arial, sans-serif; font-weight: 900; }
  .riyal-icon { width: 9px; height: 10px; object-fit: contain; vertical-align: -1px; margin-right: 2px; }
  @media print {
...
<body>${labels}</body>
  ...
  const doPrint = () => {
    ...
    let cleaned = false;
    const cleanup = () => { if (cleaned) return; cleaned = true; setTimeout(() => iframe.remove(), 400); };
  ...
  iframe.onload = () => {
    ...
    if (fontsReady) fontsReady.then(() => setTimeout(doPrint, 250));
