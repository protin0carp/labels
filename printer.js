...
function pcBuildSingleLabel(product, settings, imageCache, options = {}) {
  ...
  return `
    <div class="label">
      ...
      ${options.showPrice === false ? `` : `<div class="field price" style="${pcFieldCss(priceSettings, 'direction:ltr;')}">${pcPriceHtml(product)}</div>`}
...
async function buildPrintDocument(product, copies = 1, test = false, options = {}) {
  ...
  let labels = '';
  for (let i = 0; i < count; i++) labels += pcBuildSingleLabel(data, settings, imageCache, { showPrice });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('testPrintBtn')?.addEventListener('click', () => {
    ...
    buildPrintDocument(product, 1, true, { showPrice: document.getElementById('showPricePrint')?.checked !== false }).catch?.(err=>{ console.error(err); alert('تعذر تجهيز الطباعة.'); });
  });
});
